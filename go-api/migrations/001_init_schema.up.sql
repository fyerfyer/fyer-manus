...
CREATE TABLE IF NOT EXISTS plugins (
    ...
    tags JSONB DEFAULT '[]',
    status VARCHAR(20) DEFAULT 'inactive' CHECK (status IN ('active', 'inactive', 'error', 'loading')),
...
CREATE TABLE IF NOT EXISTS usage_quotas (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    quota_type VARCHAR(50) NOT NULL CHECK (quota_type IN ('tokens', 'api_calls', 'plugin_executions')),
